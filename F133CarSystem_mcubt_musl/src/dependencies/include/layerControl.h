...
typedef struct LayerControlOpsS
{
    ...
    int (*returnBuffer)(LayerCtrl* , VideoPicture**);
}LayerControlOpsT;
...
static int LayerReturnBuffer(LayerCtrl* l, VideoPicture** p)
{
    CDX_CHECK(l);
    CDX_CHECK(l->ops);
    CDX_CHECK(l->ops->returnBuffer);
    return l->ops->returnBuffer(l, p);
}
